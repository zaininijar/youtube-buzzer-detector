...
export function SentimentAnalysis({ sentimentData, comments }: SentimentAnalysisProps) {
          ...
          <div className="space-y-2">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              ...
              <div className="space-y-2">
                ...
                <Progress value={neutralPercent} className="h-2 bg-muted" indicatorClassName="bg-blue-500" />
              </div>
              <div className="space-y-2">
                <div className="flex justify-between">
                  <span className="text-sm">Negatif</span>
                  <span className="text-sm font-medium">{negativePercent}%</span>
                </div>
            ...
            <div className="space-y-3">
              ...
              <div className="space-y-2">
                {anomalies.map((anomaly, index) => {
                  const comment = comments.find((c) => c.id === anomaly.commentId)
                  if (!comment) return null

                  return (
                    <div key={index} className="p-3 rounded-md border bg-red-50 border-red-100">
                      <div className="flex justify-between mb-1">
                        <span className="font-medium">{comment.username}</span>
                        <span className="text-xs bg-red-200 px-2 py-0.5 rounded-full">Anomali</span>
                      </div>
