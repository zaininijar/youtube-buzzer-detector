...
export function PatternVisualizer({
  ...
  const drawPatternVisualization = (
    ...
    const cellHeight =
      (height - padding * 2) / Math.ceil(comments.length / cols);
    ...
    comments.forEach((comment) => {
      ...
      const belongsToGroups = patternGroups.filter((group) =>
        group.commentIds.includes(comment.id)
      );
    ...
    patternGroups.forEach((group, index) => {
      ...
      ctx.fillText(
        `Pola ${index + 1} (${group.commentIds.length})`,
        legendX + (isMobile ? 16 : 20),
        y
      );
    });
  };
      ...
      <CardContent>
        <div className="space-y-4">
          ...
          <div className="text-sm text-muted-foreground">
            <p>
              Visualisasi menunjukkan hubungan antar komentar berdasarkan
              kesamaan pola. Komentar yang terhubung memiliki struktur atau
              frasa yang serupa.
            </p>
          </div>
